...
The groupings of species in terms of their similar responses to the environment (the `ematrix`) are here, showing only the 4 most frequent species in each of the `ncluster = 5` groups. Here are a few rows:

```{r ecoms, eval = F}
fit$eComs[1:10,]
```

Because groups have different numbers of species, the tailing rows are packed with `NA`.
